    ...
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            m.get_root().header.add_child(
                folium.Element(f"<style>{css_content}</style>")
            )
        return True
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            m.get_root().html.add_child(
                folium.Element(f"<script>{js_content}</script>")
            )
        return True
    except Exception as e:
...
def add_preload_css(m, url, attributes=None):
    """Add a non-blocking CSS preload with a noscript fallback."""
    link_tag = f'<link rel="preload" as="style" href="{url}" onload="this.onload=null;this.rel=\'stylesheet\'"'
    if attributes:
        for attr, value in attributes.items():
            link_tag += f' {attr}="{value}"'
    link_tag += ">"
    noscript_tag = f'<noscript><link rel="stylesheet" href="{url}"></noscript>'
    m.get_root().header.add_child(folium.Element(link_tag + noscript_tag))
