...
def alternative_queries():
    ...
    queries = [
        {"name": "CLUJ substring search", "where": "indicativ_drum LIKE '%CLUJ%'"},
        {"name": "DEJ substring search", "where": "indicativ_drum LIKE '%DEJ%'"},
        {"name": "Express road category", "where": "rang_drum = 'DRUM EXPRES'"},
        {
            "name": "Express road preparation phase",
            "where": "categorie_drum LIKE '%DRUMURI EXPRES IN PREGATIRE%'",
        },
        {"name": "All express roads", "where": "categorie_drum LIKE '%EXPRES%'"},
    ]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for feature in features:
                    ...
                    if (
                        "CLUJ" in str(indicativ).upper()
                        and "DEJ" in str(indicativ).upper()
                    ):
                        print("  *** FOUND CLUJ-DEJ PROJECT! ***")
                        # Save this feature
                        filename = (
                            f"cluj_dej_project_{datetime.now().strftime('%Y%m%d')}.json"
                        )
