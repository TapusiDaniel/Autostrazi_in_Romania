...
def optimize_resources():
    ...
    if USE_BUNDLED_ASSETS:
        files_to_compress["css"].append(Path(BUNDLED_CSS_PATH).name)
        files_to_compress["js"].append(Path(BUNDLED_JS_PATH).name)
...
def bundle_app_assets():
    """Combine and minify local CSS/JS assets into app bundles."""
    os.makedirs(Path(BUNDLED_CSS_PATH).parent, exist_ok=True)
    os.makedirs(Path(BUNDLED_JS_PATH).parent, exist_ok=True)

    combined_css = ""
    for css_file in STATIC_CSS_FILES:
        try:
            with open(css_file, "r", encoding="utf-8") as f:
                print(f"Bundling CSS: {css_file}")
                combined_css += f.read() + "\n"
        except Exception as e:
            print(f"Error bundling {css_file}: {str(e)}")

    try:
        minified_css = cssmin(combined_css)
        with open(BUNDLED_CSS_PATH, "w", encoding="utf-8") as f:
            f.write(minified_css)
        print("App CSS bundle complete")
    except Exception as e:
        print(f"Error minifying app CSS: {str(e)}")

    combined_js = ""
    for js_file in STATIC_JS_FILES:
        try:
            with open(js_file, "r", encoding="utf-8") as f:
                print(f"Bundling JS: {js_file}")
                combined_js += f.read() + "\n"
        except Exception as e:
            print(f"Error bundling {js_file}: {str(e)}")

    try:
        minified_js = jsmin(combined_js)
        with open(BUNDLED_JS_PATH, "w", encoding="utf-8") as f:
            f.write(minified_js)
        print("App JS bundle complete")
    except Exception as e:
        print(f"Error minifying app JS: {str(e)}")
